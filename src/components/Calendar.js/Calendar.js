...
const Calendar = () => {
    ...
    //get the state of the links to this page so that set the data in the floating div
        ...
        document.querySelector('.today-btn').addEventListener('click', () => { 
           ...
           attachUnAvailableEventListener()
            ...
            function attachAvailableEventListener(){
                ...
                document.querySelectorAll('.calendar-days td:not([class*="date"])').forEach(td =>{
            ...
            function attachUnAvailableEventListener(){
                document.querySelectorAll('.calendar-days [class*="date"]').forEach(td =>{
                    td.addEventListener('click', (e) => {
                        setOrderTime(false);
    ...
    return ( 
        <section className="calendar-container">
            ...
            <div className="calendar-cover">
                <button onClick={ ()=> navigate(-1) } className="calendar-link">  &lt; Back</button>
                <div className="calendar-wrapper">
                    <div className="calendar-content">
                        <h1 className="calendar-title">Schedule Online</h1>
                        <div className="calendar">
                            <p className="time-zone">Jerusalem time (GMT +02:00) <span className="local-time-zone">view in my local time zone</span></p>
                            ...
                            {availableTime && <div className="unavailable" ><p>this time is unavailable</p></div>}

                            {orderTime ? <OrderTime orderTimeView ={orderTimeView }  handleOrderTimeHours={handleOrderTimeHours} orderTimeHours={orderTimeHours}/> : null}

                            <button onClick={handleView} className={`${viewTogglerBtn ? 'time-order-view' : 'inactive'} `}>Change View </button>


                            {/* if it was in mobile view port go to /info-preview (infoMobileAddInformation) component and show error if client didn't choose data */}
                            <button onClick={()=>{
                             infoObj.orderDay === '' || infoObj.orderHour ==='' ? setInfoNeeded(true) : navigate('/info-review' , {state: infoObj})
                            }} className="submit-info">Next</button>
                                                    
                            { infoNeeded &&  <p className ='calendar-error'>please select all needed information</p>}
                        </div>
            ...
            {/* floating div containing info about order : time,  title,  day , price ... . it gets the data from useLocation state object that comes from singleService component*/}
                    <div className="calendar-floating">
                        <h2 className="floating-service">{location.state.title}</h2>
                        <p className="floating-time">{location.state.time} | {location.state.price}</p>
                        {
                        orderDay &&  
                        ...
                        <button onClick={()=>{
                            infoObj.orderDay === '' || infoObj.orderHour ==='' ? setInfoNeeded(true) : navigate('/information' , {state: infoObj})
                        }} className="calendar-floating-btn">Next</button>
